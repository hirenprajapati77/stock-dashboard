...
function loadRecentOutcomes(data = null) {
    ...
    // Combine winners and losers for the list
    const winners = (d.bestSetups || []).slice(0, 2).map(o => ({ ...o, state: 'UP' }));
    const losers = (d.worstSetups || []).slice(0, 1).map(o => ({ ...o, state: 'DOWN' }));
    ...
    list.innerHTML = outcomes.map(o => {
        const pnl = parseFloat(o.pnlR || 0);
        const moveLabel = pnl >= 0 ? `+${pnl.toFixed(1)}R` : `${pnl.toFixed(1)}R`;
        const statusLabel = pnl >= 0 ? 'Target Hit' : 'SL Hit';
        ...
        <div class="flex items-center justify-between group cursor-pointer hover:bg-white/5 p-1 rounded transition-colors" onclick="document.getElementById('symbol-input').value='${o.symbol}'; fetchData();">
            <div class="flex flex-col">
                ...
                <span class="text-[8px] text-gray-500 font-bold uppercase">${o.sector || 'Equities'}</span>
            </div>
            <div class="text-right">
                <span class="text-[10px] font-black ${pnl >= 0 ? 'text-green-400' : 'text-red-400'}">${moveLabel}</span>
...
function updateExecutionEdge(data) {
    ...
    // 4. Conviction Meter
    // Widen score lookup: decision.meta_score → data.score → meta.score → summary score → 0
    // For WATCH signals on index symbols, apply a floor of 45 to avoid misleading LOW CONVICTION
    let confidence = Math.round(
        extractScore(data.decision?.meta_score,
            extractScore(data.score,
                extractScore(data.meta?.score,
                    extractScore(data.summary?.score, 0)
                )
            )
        )
    );
    // If the execution signal is WATCH but score resolved to 0, apply a floor
    if (confidence === 0 && vm.executionSignal === 'WATCH') confidence = 45;
