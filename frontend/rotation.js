...
class RotationDashboard {
    ...
    updateTargets() {
        this.particles.forEach(p => {
            ...
            const newQuad = this.getQuadrantName(p.rs, p.rm, hist.sr || 0);
    ...
    getQuadrantName(rs, rm, sr = 0) {
        if (sr > 0 && rs > 1 && rm > 0) return "LEADING";
        if (sr > 0 && rs > 1 && rm < 0) return "WEAKENING";
        if (sr < 0 && rs > 1 && rm > 0) return "IMPROVING";
        if (sr < 0 && rs < 1 && rm < 0) return "LAGGING";
        return "NEUTRAL";
    }
