...
class RotationAlertService:
    ...
    @staticmethod
    def get_quadrant(rs: float, rm: float) -> SectorState:
        if rs > 1.02 and rm > 0:
            return "LEADING"
        if rs > 1.02 and rm < 0:
            return "WEAKENING"
    ...
    @classmethod
    def detect_alerts(cls, symbol: str, curr_rs: float, curr_rm: float, entity_type: str = "sector"):
        curr_state = cls.get_quadrant(curr_rs, curr_rm)
        now = time.time()
