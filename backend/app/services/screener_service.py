    ...
    def get_screener_data(cls, timeframe: str = "1D") -> List[Dict]:
        ...
        for symbol in all_symbols:
            try:
                ...
                # IMPORTANT: trader panel must reflect current bar/session only.
                # We intentionally avoid older "last hit" fallback because it can
                # show stale green moves after today's candle turned negative.
                hit_idx = len(cond) - 1
                if hit_idx < 0 or not bool(cond.iloc[hit_idx]):
                    continue
                ...
                denom = 1.0 + (sector_return / 100.0)
                rs_sector = ((1.0 + (stock_return / 100.0)) / denom) if abs(denom) > 1e-6 else 1.0
